...
likeBtn.forEach(element => {
    element.addEventListener('click', function() {
        ...
        const likeIcon = this.querySelector('.like-button__icon');
        const likeText = this.querySelector('.like-button__label');
        const likeCounter = document.getElementById(`like-counter-${postID}`)

        if (isLiked) {
            likeIcon.classList.add('like-button--liked')
            likeText.classList.add('like-button--liked')
            post.likes++
            likedPosts.push(postID)
            console.log(likedPosts)
        } else {
            likeIcon.classList.remove('like-button--liked')
            likeText.classList.remove('like-button--liked')
            post.likes--
            const posts = [
    {
        "id": 1,
